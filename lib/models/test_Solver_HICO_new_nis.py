...
def im_detect(sess, net, image_id, Test_RCNN, keys, scores_HO, bboxes, hdet, odet, binary_result):
    ...
    for Human_out in Test_RCNN[image_id]:
        ...
        if (Human_out[1] == 'Human') and (np.max(Human_out[5]) > 0.8): # This is a valid human
            ...
            for i in range(blobs['H_num']):
                # impress nis
                pair_cnt+=1
#                print('check binary prob:',pair_cnt)
                binary_prob= binary_result[image_id][pair_cnt]
                if binary_prob[0] < 0.1 or binary_prob[1]>0.9 :
                    continue
...
def test_net(sess, net, Test_RCNN, output_dir,binary_iteration):

    binary_result_file= cfg.ROOT_DIR + '/-Results/'+ 'TIN_HICO_binary_union_binary_' + str(binary_iteration) +'/binary_score_all.pkl'
    ...
    binary_result=pickle.load(open(binary_result_file,'rb'))
    ...
    for line in glob.iglob(cfg.DATA_DIR + '/' + 'hico_20160224_det/images/test2015/*.jpg'):
        ...
        im_detect(sess, net, image_id, Test_RCNN, keys, scores_HO, bboxes, hdet, odet, binary_result)
