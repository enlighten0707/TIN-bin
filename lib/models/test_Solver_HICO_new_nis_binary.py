...
def im_detect(sess, net, image_id, Test_RCNN, keys, binary, bboxes, binary_score_all):
...
#                bboxes[classid].append(np.concatenate([hbox, obox], axis=1))
                ...
                binary_score_all[image_id].append(cls_prob_binary[i, :])
...
def test_net(sess, net, Test_RCNN, output_dir):
    ...
    for line in glob.iglob(cfg.DATA_DIR + '/' + 'hico_20160224_det/images/test2015/*.jpg'):
        ...
        im_detect(sess, net, image_id, Test_RCNN, keys, binary, bboxes, binary_score_all)
    ...
    pickle.dump(binary_score_all,open(output_dir + '/binary_score_all.pkl', 'wb'))
