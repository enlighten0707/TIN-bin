...
class VCOCOeval(object):
  ...
  def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1'):
    ...
    for aid in range(self.num_actions):
      ...
      for rid in range(len(self.roles[aid])-1):
        info = '{: >23}: AP = {:0.2f} (#pos = {:d})'.format(self.actions[aid]+'-'+self.roles[aid][rid+1], role_ap[aid, rid]*100.0, int(npos[aid]))
        print(info)
        f.write(info)
        f.write('\n')
    ...
    print('---------------------------------------------') 
    f.write('---------------------------------------------\n')
    info = 'Average Role [%s] AP = %.2f, omitting the action "point"'%(eval_type, (np.nanmean(role_ap) * 25 - role_ap[-3][0]) / 24 * 100.00)
    print(info)  
    f.write(info)
    f.write('\n')
    print('---------------------------------------------') 
  ...
  def _do_agent_eval(self, vcocodb, detections_file, ovr_thresh=0.5):
    ...
    for aid in range(self.num_actions):
      info = '{: >20}: AP = {:0.2f} (#pos = {:d})'.format(self.actions[aid], agent_ap[aid]*100.0, int(npos[aid]))
      print(info)
      f.write(info)
      f.write('\n')
    info = 'Average Agent AP = %.2f'%(np.nansum(agent_ap) * 100.00/self.num_actions)
    print(info)
    f.write(info)
    f.write('\n')
    print('---------------------------------------------')
