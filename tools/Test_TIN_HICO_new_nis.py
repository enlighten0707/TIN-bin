...
from models.test_Solver_HICO_new_nis import test_net
...
def parse_args():
    ...
    parser.add_argument('--binary_iteration', dest='binary_iteration',
            help='Select iteration for interactiveness',
...
if __name__ == '__main__':
    ...
    # pretrain model
    weight = cfg.ROOT_DIR + '/Weights/TIN_HICO/HOI_iter_' + str(args.iteration) + '.ckpt'
    print ('Human thres = ' + str(args.human_thres) + ', Object thres = ' + str(args.object_thres) + ', iter = ' + str(args.iteration) + ', path = ' + weight ) 
    output_file = cfg.ROOT_DIR + '/-Results/' + args.model + '_new_nis_' + str(args.iteration) + '_binary_' + str(args.binary_iteration)+ '/'
    ...
    test_net(sess, net, Test_RCNN, output_file, args.binary_iteration)
    sess.close()
